...
void Solver::solve_ignoring_incremental_cg() // ALgorithm 5
{
    // Get the start time
    ...
    while(true) {
        ...
        if(new_routes.empty()) { // If no new columns found, solve again with the incremental shortest path
            std::cout << "\t=== Ignoring algorithm fails to find any new routes, adding resources ===";
            new_routes = sp_solver.solve_ignoring_incremental_shortest_path(duals);
        }
